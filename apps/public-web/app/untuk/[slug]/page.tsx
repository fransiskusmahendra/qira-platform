...
const USE_CASES = {
  "usaha-jasa": {
    ...
    alt: "Alur usaha jasa dari ditemukan pelanggan hingga pekerjaan selesai",
  ...
  "retail-umkm": {
    ...
    alt: "Alur retail dan UMKM dari produk menuju pesanan yang tercatat",
  ...
  "administrasi-tim": {
    ...
    alt: "Data administrasi berubah menjadi dokumen dan status yang lebih rapi",
...
export default async function UseCasePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main>
    ...
    <section className="companyClosing simpleCompactClosing shell"><div><p className="kicker">Lihat bentuk solusinya</p><h2>Mulai dari alur yang paling penting.</h2></div><div className="closingActions"><Link className="primaryButton light" href={item.solutionHref}>Lihat solusi</Link><ContextualWhatsAppCta context={item.eyebrow} className="textLink lightText">Minta dihubungi</ContextualWhatsAppCta></div></section>
    <footer className="companyFooter shell"><div><Link className="brand" href="/">QIRA<span>.</span></Link></div><div className="footerLinks"><Link href="/">Beranda</Link><Link href="/portfolio">Portofolio</Link><Link href="/harga">Harga</Link><Link href="/privasi">Privasi</Link></div><span>QIRA · PT Rays Solusi Informasi</span></footer>
  </main>;
}
