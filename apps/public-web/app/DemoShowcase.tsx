...
export function DemoShowcase(){
  ...
  return <section className={styles.section} id="demo">
    ...
    <div className={styles.stage} key={activeId}><div className={styles.story}><span>Simulasi langsung</span><h3>{active.title}</h3><p>{active.copy}</p><Link href="/discovery">Discovery untuk usaha saya <b>↗</b></Link></div><div className={styles.preview}>{activeId==="digital-foundation"?<FoundationDemo/>:activeId==="growth-engine"?<GrowthDemo/>:<ConnectedDemo/>}</div></div>
    <div className={styles.salesGrid}><article><small>Masalah umum</small><p>{active.problem}</p></article><article><small>Solusi QIRA</small><p>{active.solution}</p></article><article><small>Hasil yang dituju</small><p>{active.outcome}</p></article><aside><div><small>Cocok diterapkan untuk</small><div>{active.uses.map(item=><span key={item}>{item}</span>)}</div></div><div><small>Estimasi implementasi</small><strong>{active.duration}</strong><Link href="/discovery">Pilih melalui Discovery</Link></div></aside></div>
