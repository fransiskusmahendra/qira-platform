...
export function FloatingWhatsApp() {
  ...
  const href = waNumber
    ? `https://wa.me/${waNumber.replace(/\D/g, "")}?text=${encodeURIComponent(
        "Halo tim QIRA, saya ingin konsultasi solusi digital yang pas untuk usaha saya..."
      )}`
    : `/coba-masalah`;

  return (
    <aside className={styles.floatingContainer} aria-label="Bantuan dan konsultasi cepat">
      <Link
        href={href}
        className={styles.floatingButton}
        onClick={() => void trackConversion("floating_wa_click")}
        aria-label="Konsultasi cepat dengan tim QIRA"
      ...
      >
        ...
        <span className={styles.buttonText}>Tanya Tim QIRA</span>
      </Link>
    </aside>
  );
}
