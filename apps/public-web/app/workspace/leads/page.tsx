...
export default async function LeadsPage(){
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.panel}><div className={styles.panelHeading}><div><p className={styles.kicker}>Lead records</p><h2>Data dan pengingat follow-up</h2></div><span className={due.length?styles.urgentBadge:styles.neutralBadge}>{due.length} due</span></div>
      {!leads?.length?<p className={styles.empty}>Belum ada lead publik.</p>:(leads??[]).map((lead:any)=>{
        ...
        {lead.status==="won"?<p><Link className={styles.primaryAction} href={`/workspace/services/onboard?${onboardingParams.toString()}`}>Lanjut onboarding customer →</Link></p>:null}
      </article>})}
    </section>
  </main>;
}
