...
export default async function ProposalPage({ params, searchParams }: ProposalPageProps) {
  ...
  const currentDecision = decisions?.find((item: any) => item.proposal_version === proposal.version);
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.panel}>
      <div className={styles.panelHeading}><div><p className={styles.kicker}>Berikutnya</p><h2>{revisionRequest ? "Revisi diminta klien" : nextLabel ?? (currentDecision?.decision === "accepted" ? "Diterima klien" : "Tidak ada aksi utama")}</h2></div>{(["approved", "shared"].includes(proposal.status)) && <a className={styles.primaryAction} href={`/workspace/proposals/${proposal.id}/export`}>PDF</a>}</div>
