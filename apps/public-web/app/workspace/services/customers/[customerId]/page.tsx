...
export default async function CustomerAccountPage({params}:{params:Promise<{customerId:string}>}){
 ...
 const membership=await supabase.from("memberships").select("role").eq("user_id",String(claims.claims.sub)).eq("status","active");if(!membership.data?.some((x:{role:string})=>x.role==="qira_admin"||x.role==="qira_consultant"))redirect("/client");
