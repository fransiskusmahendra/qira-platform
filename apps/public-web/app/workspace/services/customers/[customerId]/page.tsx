...
const model: Record<string, string> = { qira_managed: "QIRA tetap membantu", hybrid: "Dikelola bersama", customer_managed: "Pelanggan mengelola sendiri" };

const onboardingSteps = [
  ["Kickoff", "Pastikan PIC, tujuan, batas pekerjaan, dan jadwal pertama dipahami bersama."],
  ["Data & akses", "Minta hanya data atau akses yang benar-benar dibutuhkan untuk mengerjakan solusi."],
  ["Hasil pertama", "Tunjukkan hasil awal secepat mungkin agar arah bisa dikoreksi sebelum terlalu jauh."],
  ["Revisi", "Rapikan sesuai revisi dan batas pekerjaan yang sudah disepakati."],
  ["Pelunasan", "Setelah hasil utama selesai sesuai kesepakatan, lanjutkan pembayaran akhir."],
  ["Serah terima", "Pastikan akses, panduan singkat, dukungan awal, dan Care bila dipilih sudah jelas."],
] as const;

export default async function CustomerAccountPage({ params, searchParams }: { params: Promise<{ customerId: string }>; searchParams: Promise<{ onboarded?: string }> }) {
  ...
  if (!membership.data?.some((x: { role: string }) => x.role === "qira_admin" || x.role === "qira_consultant")) redirect("/client");
  ...
  if (projectIds.length) {
    ...
    deployments = result[0].data ?? [];
    domains = result[1].data ?? [];
    subscriptions = result[2].data ?? [];
    tickets = result[3].data ?? [];
  } else {
    const result = await (supabase as any).from("support_tickets").select("id,project_id,ticket_number,subject,priority,status,due_at,opened_at").eq("customer_id", customerId).order("opened_at", { ascending: false });
    tickets = result.data ?? [];
  }
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.accountHero}><div><p className={styles.kicker}>{customer.customer_type} · {customer.lifecycle_status}</p><h1>{customer.display_name}</h1><p>PIC: {customer.primary_contact_name ?? "Belum diisi"} · {customer.primary_contact_email ?? customer.primary_contact_whatsapp ?? "Kontak belum diisi"}</p></div><div><span>Estimasi pendampingan bulanan</span><strong>{rupiah.format(Math.round(activeRevenue))}</strong><small>{projects?.length ?? 0} project · {tickets.filter((x) => !["resolved", "closed"].includes(x.status)).length} tiket terbuka</small></div></section>

    {onboardingProjects.length ? <section className={styles.panel}>
      <div className={styles.panelHeading}><div><p className={styles.kicker}>Langkah setelah deal</p><h2>Selesaikan dari kickoff sampai serah terima.</h2></div><span className={styles.warningBadge}>{onboardingProjects.length} project onboarding</span></div>
      <div className={styles.followUpGrid}>{onboardingSteps.map(([title, copy], index) => <article key={title}><span>{title}</span><strong>{index + 1}</strong><small>{copy}</small></article>)}</div>
    </section> : null}

    {!projects?.length ? <section className={styles.panel}><p className={styles.empty}>Pelanggan ini belum memiliki project.</p></section> : (projects ?? []).map((project: any) => {
      ...
      return <section className={styles.accountProject} key={project.id}>
        ...
        <div className={styles.accountRecords}>
          ...
          <article><h3>Care / langganan</h3>{!ss.length ? <small>Belum tercatat</small> : ss.map((x) => <div key={x.id}><strong>{x.name} · {rupiah.format(x.amount_idr)}</strong><small>{x.status} · {x.billing_cycle} · berikutnya {date(x.next_billing_on)}</small></div>)}</article>
        ...
        <div className={styles.accountLinks}>{project.production_url ? <a href={project.production_url} target="_blank" rel="noreferrer">Buka hasil ↗</a> : null}{project.repository_url ? <a href={project.repository_url} target="_blank" rel="noreferrer">Buka repository ↗</a> : null}<Link href="/workspace/services/records">Tambah record</Link></div>
      </section>;
    })}

    {tickets.some((x) => !x.project_id) ? <section className={styles.panel}><div className={styles.panelHeading}><h2>Tiket umum pelanggan</h2></div>{tickets.filter((x) => !x.project_id).map((x) => <div className={styles.metricRow} key={x.id}><div><strong>{x.ticket_number} · {x.subject}</strong><p>{x.priority}</p></div><span>{x.status}</span></div>)}</section> : null}
  </main>;
}
