...
export default async function WorkspacePage() {
  ...
  const [{ data: memberships }, { data: proposals }, { data: discoveries }, { data: decisions }, { data: notifications }, { data: publicLeads }] = await Promise.all([
    ...
    (supabase as any).from("notifications").select("id,proposal_id,title,body,created_at,read_at").order("created_at", { ascending: false }).limit(12),
  ...
  const hotLeads = (publicLeads ?? []).filter((lead) => lead.lead_temperature === "hot" && lead.status === "new");
  ...
  const actionCount = hotLeads.length + revisions.length + waiting.length;
  ...
  return <main className={styles.page}>
    ...
    {canManage && <>
      <section className={styles.businessMetrics}>
        ...
        <article><span>Hot lead baru</span><strong>{hotLeads.length}</strong><small>Belum diproses</small></article>
      ...
      <section className={styles.panel}>
        ...
        {!hotLeads.length && !revisions.length && !waiting.length && <p className={styles.empty}>Tidak ada tindak lanjut mendesak.</p>}
        {hotLeads.slice(0, 4).map((lead) => <Link className={styles.attentionRow} href={`/workspace/leads#lead-${lead.id}`} key={`lead-${lead.id}`}><div><strong>{lead.business_name}</strong><p>{lead.full_name}</p></div><span className={styles.urgentBadge}>Hubungi</span></Link>)}
      ...
      {!!unread.length && <section className={styles.panel}>
        ...
        {unread.slice(0, 5).map((item: any) => <div className={styles.attentionRow} key={item.id}><Link href={item.proposal_id ? `/workspace/proposals/${item.proposal_id}` : "/workspace"}><strong>{item.title}</strong><p>{item.body}</p></Link><form action={markNotificationRead}><input name="notification_id" type="hidden" value={item.id}/><button type="submit">Selesai</button></form></div>)}
      </section>}
    </>}
