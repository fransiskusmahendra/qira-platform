...
export default async function DiscoveryDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string; scan?: string }> }) {
  ...
  return <main className={styles.page}>
    ...
    {query.error?.startsWith("scan") && <p className={styles.alert}>Evidence belum dapat digunakan. Hanya file berstatus clean dengan referensi pemeriksaan yang dapat diunduh.</p>}
    {query.scan && <p className={styles.success}>Hasil pemeriksaan evidence berhasil dicatat: {query.scan}.</p>}
    ...
    <section className={styles.panel}><h2>Evidence privat</h2><p>File baru dikarantina sampai hasil pemeriksaan malware eksternal dicatat oleh QIRA Admin.</p><form action={uploadEvidence}><input type="hidden" name="discovery_id" value={id}/><input name="file" type="file" accept=".pdf,.png,.jpg,.jpeg,.txt" required/><button type="submit">Upload evidence</button></form>{evidence?.map((e:any)=><div className={styles.attentionRow} key={e.id}><div><strong>{e.original_name}</strong><p>{Math.ceil(e.size_bytes/1024)} KB · {e.scan_status}{e.scan_provider?` · ${e.scan_provider} (${e.scan_reference})`:""}</p></div>{e.scan_status==="clean"&&<form action={downloadEvidence}><input type="hidden" name="evidence_id" value={e.id}/><input type="hidden" name="discovery_id" value={id}/><button>Download 60 detik</button></form>}{canRecordScan&&e.scan_status!=="clean"&&<form action={recordEvidenceScan}><input type="hidden" name="evidence_id" value={e.id}/><input type="hidden" name="discovery_id" value={id}/><select name="scan_status" required><option value="clean">Clean</option><option value="quarantined">Quarantined</option><option value="failed">Scan gagal</option></select><input name="scan_provider" placeholder="Penyedia scan" required/><input name="scan_reference" placeholder="ID/laporan hasil scan" required/><button>Catat hasil scan</button></form>}</div>)}</section>
