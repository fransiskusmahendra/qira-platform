...
export function PersonalizedDemo() {
  ...
  return <section className={styles.section} id="live-experience">
    ...
    <form className={styles.form} onSubmit={submit}>
      ...
      <label htmlFor="business-story">Tentang usaha dan kendala saat ini</label><div><textarea id="business-story" value={description} onChange={event=>setDescription(event.target.value)} minLength={10} maxLength={500} required rows={4} placeholder="Contoh: Kami menyelenggarakan pelatihan dan sertifikasi. Data peserta, jadwal batch, asesmen, dan sertifikat masih dikelola terpisah..."/><button type="submit">Lihat rekomendasi <b>→</b></button></div>
    </form>
    ...
    </div>:<div className={styles.emptyDashboard} aria-label="Preview dashboard yang akan dibuat"><div className={styles.previewHeader}><div><span>Preview solusi untuk bisnismu</span><strong>Dashboard akan menyesuaikan ceritamu</strong></div><i>Menunggu ceritamu</i></div><div className={styles.previewMetrics}><span><small>Pekerjaan aktif</small><b>12</b></span><span><small>Perlu dicek</small><b>3</b></span><span><small>Selesai</small><b>86%</b></span></div><div className={styles.previewBody}><div className={styles.previewChart}><small>Aktivitas usaha</small><div><i/><i/><i/><i/><i/><i/></div></div><div className={styles.previewTasks}><small>Alur kerja</small><span><i/>Permintaan masuk</span><span><i/>Sedang diproses</span><span><i/>Siap diselesaikan</span></div></div><p>Isi cerita singkat di atas. Dashboard ini akan berubah menjadi rekomendasi yang lebih sesuai untuk bisnismu.</p></div>}
