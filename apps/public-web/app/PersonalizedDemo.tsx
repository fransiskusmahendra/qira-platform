...
export function PersonalizedDemo() {
  ...
  return <section className={styles.section} id="live-experience">
    ...
    </div>:<div className={styles.emptyDashboard} aria-label="Contoh dashboard rental mobil"><div className={styles.previewHeader}><div><span>Contoh dashboard rental mobil</span><strong>Semua booking dalam satu tampilan</strong></div><i>Contoh tampilan</i></div><div className={styles.previewMetrics}><span><small>Mobil tersedia</small><b>8</b></span><span><small>Booking hari ini</small><b>3</b></span><span><small>Perlu konfirmasi</small><b>2</b></span></div><div className={styles.previewBody}><div className={styles.previewChart}><small>Booking minggu ini</small><div><i/><i/><i/><i/><i/><i/></div></div><div className={styles.previewTasks}><small>Status booking</small><span><i/>Pesanan masuk</span><span><i/>Cek kendaraan</span><span><i/>Konfirmasi penyewa</span></div></div><p>Ini baru contoh untuk usaha rental mobil. Setelah kamu menceritakan usahamu, QIRA akan menampilkan rekomendasi yang lebih sesuai.</p></div>}
    <p className={styles.disclosure}>Contoh di atas menggunakan data simulasi. Solusi final akan disesuaikan setelah QIRA memahami kebutuhan usahamu.</p>
  </section>;
}
